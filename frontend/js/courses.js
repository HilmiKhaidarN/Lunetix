...
// coursesData diisi dari API, fallback ke data statis jika API tidak tersedia
let coursesData = [];

// Mapping dari snake_case API ke camelCase yang dipakai di seluruh app
...
/**
 * Load courses dari API, fallback ke data statis jika gagal
 */
async function loadCoursesFromAPI() {
  try {
    const data = await CoursesAPI.getAll();
    coursesData = data.courses.map(mapCourse);
  } catch (err) {
    console.warn('[Courses] API tidak tersedia, pakai data lokal.', err);
    coursesData = coursesDataFallback;
  }
}
...
function initCourses() {
  ...
  // Load dari API lalu render
  loadCoursesFromAPI().then(() => renderCourses());
}
